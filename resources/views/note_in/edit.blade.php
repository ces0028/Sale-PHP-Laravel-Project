...
@section('content')
    ...
    <form name="form1" method="post" action="{{route('note_in.update', $row->id)}}{{$tmp}}">
    @csrf
    @method('PATCH')
        <table class="table table-bordered table-sm mymargin5">
            ...
            <tr>
                ...
                <td width="80%" align="left">
                    ...
                    @foreach($list as $product_row)
                        @php
                            $t1 = "$row->id^^$row->price";
                            $t2 = "$product_row->name($row->price)";
                        @endphp
                        @if ($product_row->id == $row->product_id)
            ...
            <tr>
                ...
                <td width="80%" align="left">
                    <div class="d-inline-flex">
                        <input type="text" name="num_in" size="20" value="{{$row->num_in}}" class="form-control form-control-sm" onchange="cal_total_price();">
                    </div>
                </td>
            </tr>
            <tr>
